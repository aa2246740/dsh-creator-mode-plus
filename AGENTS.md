...
- Keep the model-facing surface to the eight named tools, including the user-approved `dshx_hot_reload`. Every argument must remain schema-bounded and independently allowlisted in `src/runner.js`. Test the exact argv behind all eight tools; a registration-only test is insufficient.
...
- Keep boot-captured bundle removal outside the eight-tool bridge. Creator Mode+ may hand it to external `dshx plugin remove`, but must never expose that command through the managed shell or reinterpret it as `dshx_remove_plugin` watched-row success.
